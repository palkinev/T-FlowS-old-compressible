...
comp       =  'CH4:0.375, O2:1, N2:3.76'  # premixed gas composition
...
writeCSV(fcsv, ['z (m)', 'u (m/s)', 'V (1/s)', 'T (K)', 'rho (kg/m3)']
         + list(gas.speciesNames()) + ['mu (Pa s)']) #, 'D (m^2/s)'])
for n in range(f.flame.nPoints()):
    f.setGasState(n)
    writeCSV(fcsv, [z[n], u[n], V[n], T[n], gas.density()]
             +list(gas.moleFractions()) + [gas.viscosity()]) #, gas.multiDiffCoeffs()])
fcsv.close()

#for reaction rates (wdot)
fcsv = open('for_wdot.csv','w')
writeCSV(fcsv, ['z (m)']
         + list(gas.speciesNames()))
for n in range(f.flame.nPoints()):
    f.setGasState(n)
    writeCSV(fcsv, [z[n]]
             +list(f.gas.netProductionRates()))
fcsv.close()

#molarMasses
fcsv = open('molar_masses.csv','w')
writeCSV(fcsv, list(gas.speciesNames()) )
writeCSV(fcsv, list(gas.molarMasses() ) )
fcsv.close()
