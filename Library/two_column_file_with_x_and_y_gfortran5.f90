...
! known bugs: not working if total point count < 3
    ...
    type, public :: file     ! type for two column file
        character(len=:), pointer :: filename ! it has filename pointer on string of unknown size and must be given from outside
    ...
    contains ! these can be accessed through type, not class
        procedure :: somename1 => assign_file_id
        procedure :: somename2 => get_lines_count
    end type file
...
900     format (" # WARNING: ZERO VALID LINES FOUND. ZERO VALUES WILL BE USED FOR: ", A)
        ...
        allocate(character(len=len_trim(input)) :: this % filename)

        !write(*,*) "length of filename is", len_trim(input)
...
910     format (" # FILE DOES NOT EXIST: ",A)
        ...
        inquire(file = this % filename, exist=exist)
        if (exist) then
            open(this % file_id, file=this % filename, status="old", action="read")
            !write(*,800) trim(this % filename)
        else
...
285     format (" # File", A, " has homogeneous mesh ")              ! output style
        ...
        do i = 2, this % lines - 1
            ...
            if ( abs( this % dx - dx_0 ) > 1.0D-16  ) then
                write(*,275) this % filename
                return
            end if
        end do
        
        this % homogeneous_mesh = .true.
        write(*,285) this % filename
        return
