subroutine SaveVTK_VTKFortran_Lib(sub, NCsub, nameIN)
    !----------------------------------------------------------------------!
    ...
    real*8,   allocatable         :: flux_2_vtk_io(:)
    real                          :: Ua,Va,Wa,DE
    real                          :: z_test
    integer                       :: cj, j
    ...
    allocate(flux_2_vtk_io(1:NCsub)); flux_2_vtk_io(:) = 0. !< [1:NC_or_NN].

    do s=1,NS
        c1=SideC(1,s)
        c2=SideC(2,s)
        if(c2  > 0 .or. c2  < 0 .and. TypeBC(c2) == BUFFER) then
            flux_2_vtk_io(c1) = flux_2_vtk_io(c1) - Flux(s)
            if(c2  > 0) flux_2_vtk_io(c2) = flux_2_vtk_io(c2)+Flux(s)
        else
            flux_2_vtk_io(c1) = flux_2_vtk_io(c1)-Flux(s)
        end if
    end do
...
!        E_IO = vtk % xml_writer % write_dataarray( data_name = 'pressure_mean', x = P % mean(1:NCsub) )
        E_IO = vtk % xml_writer % write_dataarray( data_name = 'flux', x = flux_2_vtk_io(1:NCsub) )
...
!            E_IO = pvtk % xml_writer % write_parallel_dataarray( data_name = 'stresses_mean', data_type = 'Float64', number_of_components = 3)
        ...
        E_IO = pvtk % xml_writer % write_parallel_dataarray( data_name = 'flux', data_type = 'Float64')
...
!            E_IO = pvtk % xml_writer % write_parallel_dataarray( data_name = 'TT_mean', data_type = 'Float64')
    ...
    if ( allocated( flux_2_vtk_io          ) ) deallocate( flux_2_vtk_io          )
...
end subroutine SaveVTK_VTKFortran_Lib
